...
<div>
    <h1 class='font-medium text-2xl ml-3'>Edit Cashier</h1>
</div>
...
<div class='mx-10 -m-16'>
    <div class='flex justify-between mb-2'>
        <span class='text-[20px] text-white font-medium'>Update Cashier</span>
    </div>

    <div class='bg-white w-full rounded-lg shadow-md p-8'>
        <form method="POST" action="{{ route('cashier.update', $user->id) }}">
            @csrf
            @method('PUT') <!-- Laravel's method spoofing for PUT request -->

            <div class='mb-7'>
                <h1 class='text-[#0074C8] font-medium text-[20px]'>Cashier details</h1>
            </div>
            ...
            <div class='flex justify-end mt-5'>
                <button type="submit" class="bg-[#0074C8] text-white py-2 px-4 font-medium rounded-md">Update Cashier</button>
            </div>
        </form>
    </div>
</div>
