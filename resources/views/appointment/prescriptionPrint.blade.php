...
<div>
    <h1 class='font-medium text-2xl ml-3'>Doctors Prescription</h1>
</div>
...
<div class='mx-10 -mt-16'>
    <div class='flex justify-between mb-2'>
        <span class='text-[20px] text-white font-medium'>View Doctor Prescription</span>
    </div>
    ...
    <div class='flex gap-5 flex-wrap'>
                ...
                <div class="flex-1"> <!-- Add flex-grow to maximize the width -->
                    ...
                    <div>
                        ...
                        <div class='mt-4'> 
                        <form action="{{ route('prescription.items', $appointment->id) }}" method="POST">
                            @csrf
                            @method('PUT')

                            <!-- Diagnosis Field above the table -->
                            <div class="mb-4">
                                <label for="diagnosis" class="block mb-1 font-bold text-lg">Diagnosis</label>
                                <input type="text" id="diagnosis" class="form-control block w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    name="diagnosis" value="{{ $appointment->diagnosis ?? '' }}">
                            </div>
                            ...
                            <table class="w-full text-left border-collapse">
                                ...
                                <thead>
                                    <tr>
                                        <th class="p-2 border-t">Drug Name</th>
                                        <th class="p-2 border-t">Dosage</th>
                                        <th class="p-2 border-t">Qty</th>
                                        <th class="p-2 border-t">Action</th>
                                    </tr>
                                </thead>
                                <tbody id="invoiceTableBody">
                                    @php
                                    $drugnames = json_decode($appointment->drugname) ?? []; 
                                    $dosages = json_decode($appointment->dosage) ?? []; 
                                    $doctorqtys = json_decode($appointment->doctorqty) ?? [];
                                    @endphp

                                    @for ($i = 0; $i < count($drugnames); $i++)
                                        <tr>
                                            <td style="width: 23%;" class='pr-6 mb-2'>
                                                <input type="text" class="form-control block w-full border border-gray-300 rounded-lg px-3 py-2 mb-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                    name="drugname[]" value="{{ $drugnames[$i] }}">
                                            </td>
                                            <td style="width: 23%;" class='mb-2'>
                                                <input type="text" name="dosage[]" value="{{ $dosages[$i] }}" class="form-control block w-full border border-gray-300 rounded-lg px-3 py-2 mb-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                            </td>
                                            <td style="width: 10%;" class='mb-2'>
                                                <input type="number" class="form-control block w-full border border-gray-300 rounded-lg px-3 py-2 mb-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                    name="doctorqty[]" value="{{ $doctorqtys[$i] }}">
                                            </td>
                                            <td style="width: 10%;">
...
<script>
    document.getElementById('addRow').addEventListener('click', function () {
        ...
        newRow.innerHTML = `
            ...
            <td style="width: 23%;" class='mb-2'>
                <input type="text" name="dosage[]" value="" min="1" class="form-control block w-full border border-gray-300 rounded-lg px-3 py-2 mb-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </td>
            <td style="width: 10%;" class='mb-2'>
                <input type="number" class="form-control block w-full border border-gray-300 rounded-lg px-3 py-2 mb-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    name="doctorqty[]" value="">
                </td>
            <td style="width: 10%;">
                <button type="button" class="removeRow bg-red-500 text-white rounded-lg px-3 py-1">
                <i class="fa-solid fa-x" style="color: #ffffff;"></i>
